<template>
    <div v-for="note in notes" v-bind:key="note.id" class="bg-darkPurple hover:bg-purple-800 text-white hover:no-underline p-3 grid grid-cols-1 content-between rounded-xl h-80">
        <div class="w-full rounded">
            <div class="kartyaCim font-semibold text-lg rounded-xl w-full bg-orange p-2 text-center">{{ note.title }}</div>
            <div class="mt-6 p-2 text-gray-300 text-sm line-clamp-4">{{ note.content }}</div>
        </div>
